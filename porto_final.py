...
st.markdown("""
    ...
    [data-testid="stSidebarUserContent"] { opacity: 1 !important; }
    .stApp > div:first-child > div:first-child { background-color: transparent !important; }
    [data-testid="stStatusWidget"] { display: none !important; }
    
    /* מניעת הבהוב/החשכה בזמן עדכון */
    ...
    .stop-header { margin-bottom: 6px; display: flex; align-items: center; }
    .stop-title { font-size: 16px; font-weight: bold; color: #ffffff; }
    .stop-dist-inline { font-size: 14px; color: #aaa; font-weight: normal; margin-left: 6px; }
...
for s in sorted(nearby_stops_data, key=lambda x: x['dist']):
    bus_html = "".join([f'<div class="bus-item">🚌 {a["line"]} ({a["eta"]}m 🕒)</div>' for a in s['arrivals']]) if s['arrivals'] else '<span style="color:#888; font-size:12px;">No incoming buses</span>'
    st.markdown(f"""
        <div class="stop-card" title="{s['name']}">
            <div class="stop-header">
                <span class="stop-title">📍 {s['name']}</span>
                <span class="stop-dist-inline">({s['dist']}m)</span>
            </div>
            <div class="arrival-row">{bus_html}</div>
        </div>
    """, unsafe_allow_html=True)
