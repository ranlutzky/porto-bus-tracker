...
active_lines = [str(e.get('name', {}).get('value', '')).split()[1] for e in buses_raw if len(str(e.get('name', {}).get('value', '')).split()) >= 2]
unique_lines = sorted(list(set([l for l in active_lines if l.isdigit()])), key=int)
target = st.selectbox("Line:", ["Nearby Buses"] + unique_lines, label_visibility="collapsed")

loc = get_geolocation()
user_lat, user_lon = (loc['coords']['latitude'], loc['coords']['longitude']) if (st.session_state.location_mode == 'gps' and loc) else st.session_state.get('map_center')
...
st_folium(m, width=None, height=450, key=f"map_v23", use_container_width=True)
