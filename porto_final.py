...
# --- טעינת תחנות מקובץ ה-JSON החדש (v53) ---
def get_all_stops():
    # בודק אם הקובץ קיים בתיקייה
    if os.path.exists('bus_stops.json'):
        try:
            with open('bus_stops.json', 'r', encoding='utf-8') as f:
                return json.load(f)
        except:
            return [] # אם יש שגיאה בקובץ, מחזיר רשימה ריקה כדי לא לקרוס
    return []
...
STATIC_STOPS = get_all_stops() # קורא מה-JSON
...
st_folium(m, width=None, height=400, key="map_v53", use_container_width=True)
...
for s in sorted(nearby_stops_data, key=lambda x: x['dist']):
    ...
    st.markdown(f'<div class="stop-card"><div class="stop-header"><div class="stop-title">📍 {s["name"]}</div><div class="stop-dist-tag">{s["dist"]}m</div></div><div class="arrival-row">{bus_html}</div></div>', unsafe_allow_html=True)
