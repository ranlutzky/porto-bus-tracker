...
# 3. CSS (v23) - טקסט בהיר על רקע כהה
st.markdown("""
    <style>
...
st_folium(m, width=None, height=450, key=f"map_v23_final", use_container_width=True)

# --- תיבת מרחק (ממש מתחת למפה) ---
if display_buses:
    closest = min(display_buses, key=lambda x: x['dist'])
    st.markdown(f'''
        <div class="distance-box">
            🚍 Closest: <b>Line {closest["line"]}</b> is <b>{closest["dist"]:.2f} km</b> away
        </div>
    ''', unsafe_allow_html=True)
