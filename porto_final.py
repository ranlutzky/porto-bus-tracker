...
def get_porto_entities(e_type):
    url = f"https://broker.fiware.urbanplatform.portodigital.pt/v2/entities?type={e_type}&limit=1000"
    try:
...
# --- שליפת נתונים ---
buses_raw = get_porto_entities("bus")
stops_raw = get_porto_entities("busStop")
...
# תיבת מידע עליונה (v23)
if display_buses:
    closest = min(display_buses, key=lambda x: x['dist'])
    st.markdown(f'<div style="background-color: #262730; border: 1px solid #00ccff; padding: 10px; border-radius: 5px; text-align: center; color: white; margin-bottom: 10px;">🚍 Closest: <span style="color: #ffff00; font-weight: bold;">Line {closest["line"]}</span> is <span style="color: #ffff00; font-weight: bold;">{closest["dist"]:.2f} km</span> away</div>', unsafe_allow_html=True)
...
for s in stops_raw:
    ...
    if s_dist <= 0.3: # רדיוס 300 מטר
        ...
        folium.CircleMarker(
            ...
            fill_color='#cccccc',
            fill_opacity=0.7,
            popup=folium.Popup(f"🚏 {s_name}", max_width=150)
        ).add_to(m)

# 2. הוספת אוטובוסים
for b in display_buses:
    popup_html = f'<div style="text-align:center;"><b>Line {b["line"]}</b><br><a href="https://stcp.pt/en/line?line={b["line"]}" target="_blank">Route</a></div>'
...
st_folium(m, width=None, height=450, key=f"map_v32", use_container_width=True)
